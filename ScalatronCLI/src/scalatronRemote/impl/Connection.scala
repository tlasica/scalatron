...
case class Connection(httpClient: HttpClient, hostname: String, port: Int, verbose: Boolean = false) {
  ...
  def GET_json(resource: String): JSonOpt = {
    ...
    if (statusCode != HttpStatus.SC_OK) {
      throw new HttpFailureCodeException(statusCode, response.getStatusLine.getReasonPhrase, "GET on '" + uri.toString + "' failed; HTTP code: " + statusCode + ", reason: " + response.getStatusLine.getReasonPhrase)
    }
  ...
  def POST_json_json(resource: String, jsonString: String): JSonOpt = {
    ...
    if (statusCode != HttpStatus.SC_CREATED && statusCode != HttpStatus.SC_OK) {
      throw new HttpFailureCodeException(statusCode, response.getStatusLine.getReasonPhrase, "POST on '" + uri.toString + "' failed with HTTP code " + statusCode + ", reason: " + response.getStatusLine.getReasonPhrase)
    }
  ...
  def DELETE(resource: String) {
    ...
    val statusCode = response.getStatusLine.getStatusCode
    if (statusCode != HttpStatus.SC_NO_CONTENT) {
      throw new HttpFailureCodeException(statusCode, response.getStatusLine.getReasonPhrase, "DELETE on '" + resource + "' failed with HTTP code " + statusCode + ", reason: " + response.getStatusLine.getReasonPhrase)
    }
  }
